...
An **open note** persists round to round, accumulating the exchange, until the
reviewer **settles** it — unlike a one-shot note, which would vanish after one
rewrite and force a re-flag.
...
span, so the conversation keeps its referent even if the span is later
rewritten away.
...
3. The re-parse re-presents still-open threads on their cards with the full
   prior exchange and a reply box, so the reviewer continues the conversation
   across rounds, GitHub-style.
...
A decline resolves nothing: the thread stays unresolved, re-presents next round
with your grounds, and the section stays held. The reviewer then settles it
(accepts) or replies, which returns it to `open` and means they insist.
**Insisting wins**: apply the change, and record it with `--response`. There is
no second decline on the same thread; `open_notes.py` refuses one and the round
does not ship until you comply.
...
- latest turn `suggestion` → **apply its `replacement` verbatim** to the
  ...
  second instruction. An un-anchored suggestion names no span: treat it as a
  `changes` turn scoped to the section.
...
An `info` discussion becomes a change when the reviewer **escalates** by
switching their reply to *request changes* — that turn arrives as a comment on
the same `cid` with `type: "changes"`, so the rule above applies it. A reply
that merely continues the discussion keeps the thread's `info` type.
...
threads: no store, no `open_notes` on any card, no Open notes ledger section.
