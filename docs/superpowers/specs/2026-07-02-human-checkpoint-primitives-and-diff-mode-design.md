...
**Special cases:**
- Binary file diff: emit one section per file with content `Binary file changed — no content to review`, verdict auto-approved
...
- **Syntax highlighting theme**: `highlight.js` default theme vs. a custom CSS override matching viva's token colors. The `diff` language class has standard `hljs-addition`/`hljs-deletion` classes — override these to map to `--teal`/`--orange` in the SPA's style block.
- **Binary file auto-approve**: The spec says binary hunks are auto-approved. Confirm this is the right behavior vs. presenting them as a card with a fixed "no content" body and requiring explicit approval.
