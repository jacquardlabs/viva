...
The store lives at `.viva/preferences.json`; `scripts/preferences.py` is its primary writer, and (issue #142) the running review server's `POST /preferences/mute` route is a second, narrower one — it only ever flips an existing preference to `muted`, from an in-page panel the reviewer can open without leaving the tab, via the same `set_status()` this script's own `set --status muted` calls. It never creates a preference or promotes one. Unlike the round files, the store is **not** cleared by the round-1 state reset, so it persists across sessions; it is gitignored, so learned preferences are a reviewer's own (per clone), not shared. It is plain JSON — the human can open and edit it directly, and muting (from the panel, or `preferences.py set --status muted` in a terminal) retires a bad one; un-muting stays CLI-only (`set --status standing`). Preferences are **suggestions the agent applies, not rules**: a pre-applied fix still rides a normal rewrite to the human, and a pre-flag is an advisory annotation that never gates a verdict. Nothing is auto-accepted.
